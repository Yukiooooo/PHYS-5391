...
parser.add_argument('-N', help='Number of the IMF files. '+
                    'Defaults to 0.', type=int)
...
path = args.imffolder
dirs = glob.glob(path+'*.dat')

#       select one file for test
for ifile,filename in enumerate(sorted(dirs)):  # dirs[::100]
    if ifile > args.N:  #if ifile%5 !=0: used for jump reading with n=5n+1  ifile >4:
        continue
    ...
    with open('my_test_epochs.txt', 'a') as f: # change w to a
    #    f.write('Substorm onsets created from the Minimal Substorm Model (MSM)\n')
    #    f.write(f'Input file used: {filename}\n')
